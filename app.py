...
class Backend:
    ...
    def trigger_group(self, count, mode, duration):
        ...
        self.log(f"Запущена группа alarm: count={min(count, len(self.sensors))}, mode={mode}, duration={duration} сек")
...
async def broadcaster():
    """Push status to all WebSocket clients every second."""
    while True:
        try:
            ...
            debug_print(f"Broadcaster error: {e}")
        await asyncio.sleep(1)
